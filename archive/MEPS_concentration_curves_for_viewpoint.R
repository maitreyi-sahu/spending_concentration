...
pacman::p_load(data.table, tidyverse, dplyr, openxlsx, arrow, RColorBrewer, ggplot2)
...
sample_denoms <- sample_denoms[,.(pop = sum(pop), n_obs = sum(n_obs)), by = c('year_id')]

# Load data from data processing
data <- rbindlist(lapply(list.files(meps_data_path, full.names = T), read_parquet), fill = T)

# Mimic DETRUNC processing and drop rows with missing info
drop_cols <- c(   "toc",   "age_group_years_start",   "age_group_id",  "pri_payer",  "sex_id",  "code_system",  "dx_level",  "dx")
...
data <- data[,.(year_id, bene_id, age_group_years_start, sex_id, toc, encounter_id = claim_id, pay = tot_pay_amt, survey_wt)]
...
bene_key <- unique(data[,.(bene_id, age_group_years_start, sex_id, survey_wt, year_id)])
#bene_key[,.N, by = c('bene_id', 'year_id')][N > 1]          

# Aggregate spend by bene (so across encounters) by toc and all-toc
data_t <- data[,.(pay = sum(pay)), by = c('bene_id','toc', 'survey_wt', 'year_id')]
data_nt <- data[,.(pay = sum(pay)), by = c('bene_id', 'survey_wt', 'year_id')][, toc := 'all']
...
if(use_wgts == T){
  ...
  if(include_sample_pop == T){
    ...
    sample_w_encounters <- merge(sample_w_encounters, sample_denoms, by = c('year_id'))
    sample_w_encounters[, survey_wt := pop - tot_wt]
    sample_w_encounters[, prop := survey_wt/pop]
    
    sample_w_encounters <- sample_w_encounters[,.(year_id, toc, survey_wt, pay = 0, bene_id = 'no-encounters')]
  ...
  if(include_sample_pop == T){
    ...
    sample_w_encounters <- sample_w_encounters[,.(year_id, toc, N = add_obs, pay = 0, bene_id = 'no-encounters')]
...
closest_percent <- rbindlist(lapply(0:100, function(p){
  #message(p)
...
closest_percent[, `:=` (min_percentile = NULL, dist_from_p = NULL, min_dist = NULL, max_percentile = NULL)]
closest_percent <- unique(closest_percent)

#-----------
# NUMBER PLUG - top 10% of users account for >X% of spending
#-----------
check <- closest_percent[toc == 'RX' & year_id == max(year_id) & percentile > 89.9999 & percentile < 90.1 ]
paste0("top 10% of users in MEPS account for ", round(100-check$percent_tot_spend, digits = 2), "% of total spending")
...
if(gini_plots == T){
  pdf(paste0(save_dir, "/meps_concentration_curves_",
...
for(t in sort(unique(plot_df$toc))){
  ...
  gini_by_year <- rbindlist(lapply(years, function(y){
    ...
    tmp_plot_data <- rbind(tmp_plot_data, data.table(percentile = c(0,100), toc = t, year_id = y, percent_tot_spend = c(0,100)), fill = T)
    ...
    line_fun <- approxfun( tmp_plot_data$percentile, tmp_plot_data$percent_tot_spend, yleft=0, yright=0)
    # tmp_plot_data$pred_percent <- line_fun(tmp_plot_data$percentile)
    
    get_gini <- tryCatch(
      expr = {
        integrate(line_fun, min(tmp_plot_data$percentile), max(tmp_plot_data$percentile) )$value
      },
      error = function(e){ 
        return(NA)
      })
    ...
    if(!is.na(get_gini)){
      b <- integrate(line_fun, min(tmp_plot_data$percentile), max(tmp_plot_data$percentile) )$value
      c <- (max(tmp_plot_data$percentile) * max(tmp_plot_data$percent_tot_spend))/2
      a <- c-b
    ...
    return(data.table('year_id' = y, 'gini' = round(gini_coef_approx, digits = 2)))
  ...
  if(include_sample_pop == T){
    ...
    if(t == 'all'){
      ...
      x_axis_lab <- paste0('Percent of people with at least one ', t, " encounter")
    }
  }
  
  
  plot <- ggplot(tmp_plot_data[year_id %in% plot_years], aes(x = percentile, y = percent_tot_spend))+
    geom_line()+
    facet_wrap(~year_id)+
    geom_ribbon(aes(ymin=percent_tot_spend, ymax=percentile), fill="grey", alpha=0.5) +
    geom_text(data = gini_by_year[year_id %in% plot_years], aes(x = 3, y = 15, label = gini), size = 3.5)+
    labs(x = x_axis_lab, y = 'Percent of spending', title = paste0('Spending equality ', ifelse(t == 'all', "across all TOCs", paste0("in ", t))))+
...
pdf(paste0(save_dir, "/lorenz_curves_overlay",
           ifelse(use_wgts,"","_notweighted_"),
           ifelse(include_sample_pop, "", "_peoplewithenc"),
           max(iso_years),
           ".pdf"), width = 6, height = 4.5)  # 8,6 


# calculate the 90%
check <- plot_lorenz_curve_data[toc == 'RX' & percentile > 89.9999 & percentile < 90.1 & year_id %in% iso_years]
percent_key <- check[, ten_percent_user_spend := paste0(round(100-percent_tot_spend, digits =0), "%") ]
percent_key[, percentile := percentile + 3.5]
percent_key[, percent_tot_spend_plot  := percent_tot_spend  - 2]

# Make plot
plot <- ggplot(plot_lorenz_curve_data[year_id %in% iso_years], 
  ...
  geom_segment(data = percent_key, aes(y = percent_tot_spend,x = 0, xend = 90, yend = percent_tot_spend, color = factor(year_id)), linetype = 2)+
  geom_text(data = percent_key, aes(label = ten_percent_user_spend, color = factor(year_id), y = percent_tot_spend_plot), show.legend = FALSE)+
  scale_color_brewer(palette = 'Set1')+
  scale_fill_brewer(palette = 'Set1')+
  geom_ribbon(aes(ymin = percent_tot_spend, ymax = percentile, fill = factor(year_id)), alpha = 0.3) +
  labs(x = 'Percent of respondents in the Medical Expenditure Panel Survey [MEPS]', 
       y = "Percent of retail prescription drug spending", 
       title = paste0("Panel A: Concentration of retail prescription drug \n spending, 2000 versus 2022"),
       color = "Year",
       fill = "Year") +
  theme_bw()
print(plot)
...
write.csv(plot_lorenz_curve_data, paste0(save_dir, "/lorenz_curves_overlay_data.csv"), row.names = F)
...
pdf_name <- paste0(save_dir, "/gini_coefficient_timetrend_",
                   ifelse(use_wgts,"","notweighted_"),
                   ifelse(include_sample_pop, "", "peoplewithenc"),
                   max(iso_years))


toc_gini_timetrend[, toc_plot := plyr::revalue(toc, 
                                               c( "AM" ="Ambulatory", 
                                                  "DV" = 'Dental',
...
base_colors2 <- c("#B3E2CD", "#FDCDAC", "#ACACE6", "#F4CAE4", "#AECCE4", "#00008C")

pdf(paste0(pdf_name, ".pdf"), width = 6, height = 4.5)
plot <- ggplot(toc_gini_timetrend[toc != 'all'], aes(x = year_id, y = gini, color = toc_plot, fill = toc_plot))+
  ...
  labs(x = 'Year', y = 'Gini coefficient', title = paste0('Panel B: Inequality index for health care spending over time \n by type of care, 2000 to 2022'), color = '', fill = '')
print(plot)
dev.off()

write.csv(toc_gini_timetrend, paste0(save_dir, "/gini_coef_timetrend_data.csv"), row.names = F)
